...
export class Lights {
  ...
  numLights = 500;
  static readonly maxNumLights = 70000;
  ...
  clusterComputerPipeline: GPUComputePipeline;
  ...
  constructor(camera: Camera) {
    ...
    this.clusterBuffer = device.createBuffer({
      ...
      size:
        4 * shaders.constants.Nx * shaders.constants.Ny * shaders.constants.Nz,
      usage: GPUBufferUsage.STORAGE,
    });

    this.lightIndicesBuffer = device.createBuffer({
      label: "light indice buffer",
      size:
        4 *
        shaders.constants.Nx *
        shaders.constants.Ny *
        shaders.constants.Nz *
        shaders.constants.clusterMaxLights,
      usage: GPUBufferUsage.STORAGE,
    });
    ...
    this.clusterComputeBindGroup = device.createBindGroup({
      ...
      entries: [
        ...
        {
          binding: 3,
          resource: { buffer: this.lightIndicesBuffer },
        },
      ],
    });

    this.clusterComputerPipeline = device.createComputePipeline({
  ...
  doLightClustering(encoder: GPUCommandEncoder) {
    ...
    computePass.setPipeline(this.clusterComputerPipeline);

    computePass.setBindGroup(0, this.clusterComputeBindGroup);

    const groupX = Math.ceil(shaders.constants.Nx / shaders.constants.Wx);
    const groupY = Math.ceil(shaders.constants.Ny / shaders.constants.Wy);
    const groupZ = Math.ceil(shaders.constants.Nz / shaders.constants.Wz);
    computePass.dispatchWorkgroups(groupX, groupY, groupZ);

    computePass.end();

    // dont submit queue let submission happen at end of draw call
  }
